 ...
 */
export const getPluralSymbols = (number: number): string => {
    ...
    if (inputElement.validity.patternMismatch) {
        ...
        if (!/.{8,}/.test(inputElement.value)) {
            ...
            if (minLength) {
                ...
                errorMessage = `Как минимум ${getPluralSymbols(
                    parseInt(minLength)
                )}. Осталось ввести еще ${getPluralSymbols(symbolsLeft)}`;
            }
        }
 ...
 */
export const getErrorMessage = (inputElement: HTMLInputElement): string => {
 ...
 */
export const hasInvalidInput = (inputList: HTMLInputElement[]): boolean => {
 ...
 */
export const setEventListeners = (formElement: HTMLFormElement): void => {
 ...
 */
export const enableValidation = (): void => {
