...
//mainCard.querySelector('.element__like-button').addEventListener('click', function (evt) {
...
function cardCreation (item) {
    ...
    const mainCard = document.querySelector('.element');
    ...
    //const imageDetail = detailCard.querySelector('.element__image');
    //imageDetail.src = item.link;
    //imageDetail.alt = item.name;
...
popupPlusCreate.addEventListener('submit', () => {
    Create(popupMainPlusButton)
});

//Увеличенная картинка при клике на кнопку мыши
...
popupImageClose.addEventListener('click', popupToggle);
